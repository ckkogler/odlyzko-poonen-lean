...
assert 'authors' not in metadata['sources'][0], 'Anonymous source decision'
...
assert len(re.findall(r'\bsorry\b',code))==20
...
visit('Solution')
assert 'OdlyzkoPoonen' in seen and len(seen)==184
...
print(f'Challenge: {len(text.splitlines())} lines, {len(text.encode())} bytes; 20 independent claims, no definition holes.')
print('Solution: all 182 preserved proof modules, default root and Solution; no Challenge import or unchecked shortcut.')
print('All 20 claims aligned and covered by full Verification.lean; exact public Git pins and contained reuse.')
